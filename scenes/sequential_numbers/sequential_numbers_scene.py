...
class SequentialNumbersScene(GameScene):
    ...
    def __hide_sequency(self) -> None:
        ...
        for i in range(len(sequency)):
            ...
            self.__blocks[y][x].set_text("")

    def __enable_sequency(self) -> None:
        # Deshabilitar todos los bloques
        for i in range(len(self.__blocks)):
            for j in range(len(self.__blocks[i])):
                self.__blocks[i][j].enable()

    def __disable_sequency(self) -> None:
        # Deshabilitar todos los bloques
        for i in range(len(self.__blocks)):
            for j in range(len(self.__blocks[i])):
